...
def main():
    ...
    sorted_average_salary = sorted(filtered_average_salary_by_industry, key=lambda x: x.average, reverse=True)
    top_5_industries = []
    for entry in sorted_average_salary[:5]:
...
def group_by_attribute(objects, property):
    '''
        params:
            ...
            property: Str - whichever object property we want to group by
        return:
